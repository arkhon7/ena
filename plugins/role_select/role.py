...
class ReactionRole:
    def __init__(self, bot: lightbulb.BotApp, role_id: int, emoji_name: str, message_id: int) -> None:
    ...
    async def add_role_to_member(self, user_id: int, guild_id: int):
        await self.bot.rest.add_role_to_member(guild=guild_id, role=self.role_id, user=user_id)

    async def remove_role_from_member(self, user_id: int, guild_id: int):
        await self.bot.rest.remove_role_from_member(guild=guild_id, role=self.role_id, user=user_id)
...
class ReactionRoleHandler:
    ...
    async def add_role_to_member(self, emoji_name: str, user_id: int):
        for reaction_role in self.reaction_roles:
            if reaction_role.emoji_name == emoji_name and reaction_role.message_id == self.message_id:
...
def create_reaction_role(bot: lightbulb.BotApp, emoji_name: str, role_id: int, message_id: int) -> ReactionRole:
...
def create_reaction_role_handler(
    ...
    role_handler = ReactionRoleHandler(bot, guild_id=guild_id, message_id=message_id, reaction_roles=reaction_roles)
    return role_handler
